...
exports.signup = (req, res, next) => {
    ...
    if (!schema.validate(req.body.password)) {
        ...
        return res.status(400).json({ message: "Le mot de passe doit contenir au moins 8 caractères, un chiffre, une majuscule, une minuscule et ne pas contenir d'espace !" });
    }
    bcrypt
                ...
                .then((valid) => {
                    ...
                    return res.status(200).json({
                        ...
                        token: jwt.sign(
                            ...
                            { userId: user._id },
                            process.env.JWT_SECRET_TOKEN, { expiresIn: "3600s" }
                        ),
                    });
                })
                .catch((error) => res.status(500).json({ error }));
